...
_xvseq.Seq.prototype.op = function (operator, other) {
    ...
    if (typeof operator == "string") {
        ...
        if (/^and|or|not$/.test(operatorName)) {
            ...
            opfn = comp ? _comp.bind(new Error(), operatorName, invert) : _op.bind(null, operatorName, invert);
        }
        if (comp) {
            $a = data($a,false);
...
function data($a,asString) {
    if (!(0, _xvnode._isNode)($a)) {
        if ((0, _xvseq._isSeq)($a)) return $a.map(function (_) {
